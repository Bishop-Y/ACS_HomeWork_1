...
writeResult:
	...
	mov	QWORD PTR -32[rbp], rsi # второй входной параметр из функции (указатель на начало массива B)
	lea	rsi, .LC0[rip]
	lea	rdi, .LC1[rip]
	call	fopen@PLT
	mov	QWORD PTR -8[rbp], rax
	mov r14, QWORD PTR -8[rbp] # r14 = указатель на output
	mov	r13d, 0 # DWORD PTR -12[rbp] => r13d (итерируемая переменная index)
	jmp	.L2
.L3:
